...
filename = r"data/bunny_oded.obj"
if len(sys.argv) > 1:
    filename = sys.argv[1]
...
params = fracture.FractureModesParameters(num_modes=3, verbose=True, d=1)
...
write_obj_name = f"{os.path.splitext(base)[0]}{dt_string}.ply"
...
def callback():
    ...
    if psim.Button("Show modes"):
        ...
        for i in range(UU.shape[1]):
            ...
            ps_vol.append(
                ps.register_surface_mesh(mesh_name, vertices=fine_vertices_1d + (i // 4) * off_y + (i % 4) * off_x,
                                         faces=fine_triangles_1d))
            ps_vol[i].add_scalar_quantity(scalar_map_name, labels_fine_1d[:, i], enabled=True, cmap='phase',
                                          vminmax=(0, np.max(labels_fine_1d[:, i]) + 1.0))
    ...
    if psim.Button("Impact mode"):
        ...
        if show_full_impact:
            ps_impact_projected_mesh = ps.register_volume_mesh("projected impact mesh",
                                                               modes.exploded_vertices - off_tets_x_exploded,
                                                               tets=modes.exploded_elements)
        else:
            ps_impact_projected_mesh = ps.register_volume_mesh("projected impact mesh", modes.vertices - off_tets_x,
                                                               tets=modes.elements)
        ps_fracture_mesh = ps.register_surface_mesh("fracture mesh", modes.fine_vertices - 2 * off_x,
                                                    modes.fine_triangles)
        ps.reset_camera_to_home_view()
    ...
    if showing_impact:
        ...
        if psim.Button("Generate random impact"):
            ...
            if show_full_impact:
                ...
                ps_impact_projected_mesh.update_vertex_positions(
                    modes.exploded_vertices - off_tets_x_exploded + 0.002 * modes.tet_to_vertex_matrix @ modes.piece_to_tet_matrix @ np.reshape(
                        modes.projected_impact, (-1, 3), order='F'))
            else:
                if direction.shape[0] == 1:
                    ps_impact_mesh.add_scalar_quantity("impact", np.squeeze(modes.impact_vis), enabled=True,
                                                       defined_on='vertices')
                    ps_impact_projected_mesh.add_scalar_quantity("projected impact",
                                                                 modes.piece_to_tet_matrix @ modes.projected_impact,
                                                                 enabled=True, defined_on='cells')
                else:
                    ps_impact_mesh.add_scalar_quantity("impact",
                                                       np.linalg.norm(np.reshape(modes.impact_vis, (-1, 3), order='F'),
                                                                      axis=1), enabled=True, defined_on='vertices')
                    ps_impact_projected_mesh.add_scalar_quantity("projected impact", np.linalg.norm(
                        modes.piece_to_tet_matrix @ np.reshape(modes.projected_impact, (-1, 3), order='F'), axis=1),
                                                                 enabled=True, defined_on='cells')
            ps_fracture_mesh.add_scalar_quantity("fracture", modes.fine_vertex_labels_after_impact, enabled=True)
